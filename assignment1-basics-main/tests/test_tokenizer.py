...
def get_tokenizer_from_vocab_merges_path(
    ...
    # GPT-2 tokenizer 对字节使用了重映射的 unicode 编码。
    # 我们直接返回原始字节，这样不强制学生使用任何特定的编码方案。
...
def test_overlapping_special_tokens():
    ...
    ids = tokenizer.encode(test_string)
    tokenized_string = [tokenizer.decode([x]) for x in ids]
    # 确保双 <|endoftext|><|endoftext|> 作为单个 token 被保留
...
@pytest.mark.skipif(
    ...
    reason="rlimit support for non-linux systems is spotty.",  # 非 Linux 系统的 rlimit 支持不稳定。
)
...
def test_encode_memory_usage():
    """
    # 我们预期此测试会失败，因为 Tokenizer.encode 不要求内存高效。
...
def _encode_iterable(tokenizer, iterable):
    # 我们将 tokenizer.encode_iterable 放入单独的函数中，以便可以限制
    # 仅此函数的内存使用。我们将内存限制设置为 1MB。
    """
...
def _encode(tokenizer, text):
    # 我们将 tokenizer.encode 放入单独的函数中，以便可以限制
    # 仅此函数的内存使用。我们将内存限制设置为 1MB。
    """
